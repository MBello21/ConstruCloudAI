import { Save, Loader, CheckCircle, AlertCircle, Eye } from "lucide-react";
import { Button } from "../../../shared/components/Button";
import { useAuth } from "../../auth/context/AuthContext";
import { useEmpresaSection } from "../hooks/useEmpresaSection";

export const EmpresaSection = () => {
  const { user } = useAuth();
  const { formData, isLoading, isSaving, error, success, isDirty, handleChange, handleSubmit } =
    useEmpresaSection();
  ...
  if (isLoading) {
    return (
      <div className="bg-white rounded-lg border border-slate-200 p-8 flex items-center justify-center py-12">
        <Loader className="w-5 h-5 animate-spin text-blue-950" />
      </div>
    );
  }
      ...
      <div className="mb-8">
        ...
        {!isAdmin && (
          <p className="text-xs text-amber-600 mt-2 flex items-center gap-1">
            ...
            Solo administradores pueden editar estos datos
          </p>
        )}
      </div>
